...
using TurkiyeServices;
using static TurkiyeServices.KPSPublicSoapClient;
...
{
    KPSPublicSoapClient soapTurkeyClient = new KPSPublicSoapClient(EndpointConfiguration.KPSPublicSoap12);
    ...
    public async Task<IActionResult> AddAsync([FromBody] User user)
    {
        ...
        try
        {
            identityNumber = Convert.ToInt64(user.IdentificationNumber);
        }
        catch
        {
            return BadRequest("Please IdentificationNumber just write with number");
        }
        var result = await soapTurkeyClient.TCKimlikNoDogrulaAsync(identityNumber, user.FirstName, user.LastName, user.BirthYear);
